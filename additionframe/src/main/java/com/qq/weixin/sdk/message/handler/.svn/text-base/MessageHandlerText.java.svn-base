...
import com.qq.weixin.sdk.message.filter.FilterText;
import com.qq.weixin.sdk.message.filter.FilterTemplate;
...
public class MessageHandlerText extends MessageHandlerHelper {
	...
	public Message handleSpecialMessage(Message message) {
		...
		filterChain.addFilter(new FilterText());
		// add this,so the next line does not have to verify whether result is
		// null or not
		filterChain.addFilter(new FilterDefaultResult());
